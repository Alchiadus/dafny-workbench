  ...
  static parse(textEditorPath, stdout) {
    ...
    stdoutLines.forEach((line) => {
      DafnyOutputParser.lineParsers.forEach((lineParser) => {
        ...
        if (message !== null) {
          if (message.excerpt.includes('[Owicki-Gries] Global Correctness: this is the violating statement')) {
            // Link statement to assertion.
            messages[messages.length - 1].reference = message.location;
          }
