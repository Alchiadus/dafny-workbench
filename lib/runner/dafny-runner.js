...
export default class DafnyRunner extends EventEmitter {
  ...
  async startDafy() {
    ...
    // Warning: ChildProcess.execFile has a default maxBuffer of 200KB.
    this.dafnyExecutionStartTime = performance.now();
    this.dafnyProcess = ChildProcess.execFile(this.executablePath, this.executableArguments.concat([tmpFilePath]), (error, stdout, stderr) => {
